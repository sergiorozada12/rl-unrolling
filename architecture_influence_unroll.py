...
log_every_n_steps = 1
K = 5
group_name = f"architecture_comparison_unroll_K{K}"
...
def run(g, N_unrolls, Exps, q_opt, group_name, use_logger=True, log_every_n_steps=1, verbose=False):
    ...
    for i, n_unrolls in enumerate(N_unrolls):
        ...
        for j, exp in enumerate(Exps):
            ...
            if verbose:
                print(f"    📈 Detailed: Unrolls={n_unrolls}, Architecture={exp['args']['architecture_type']}, Err1={err1[j,i]:.3f}, Bell_err={bell_err[j,i]:.3f}")
...
def main():
    print("="*70)
    print("🚀 STARTING ARCHITECTURE COMPARISON - UNROLL INFLUENCE EXPERIMENTS")
    print("="*70)
    ...
    # Set up experiments
    n_runs = 15
    errs1_arch = np.zeros((n_runs, len(Exps_architectures), N_unrolls.size))
    errs2_arch = np.zeros((n_runs, len(Exps_architectures), N_unrolls.size))
    ...
    for g in range(n_runs):
        ...
        errs1_arch[g], errs2_arch[g], bell_errs_arch[g] = run(
            g, N_unrolls, Exps_architectures, q_opt, group_name, use_logger, log_every_n_steps, verbose
        )
